...
export function addStrings(x: string, y: string) {
  return x + y;
}
...
export function attempt(func: Function, ...args: Array<any>) {
  try {
    return func(...args);
  } catch (err) {
    return err;
...
// array of results.
export function times<T>(n: number, iteratee: Function): Array<T> {
